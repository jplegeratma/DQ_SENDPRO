...
FROM (
  ...
  FROM (
    SELECT *
    FROM INF_B_SENDPRO_CLAIMS_DQ_7_837I_UNPIV_DETAIL
    WHERE NUMDTL = 1
    )
)
WHERE rnk <= 10
...
DROP VIEW MHTEAM.DWDQ.INF_B_SENDPRO_CLAIMS_DQ_7_837I_DETAIL

/*
CREATE VIEW MHTEAM.DWDQ.INF_B_SENDPRO_CLAIMS_DQ_7_837I_DETAIL
AS
SELECT DISTINCT RUN_DATE, FILENAME, CLAIM_TYPE, RECORD_TYPE, patientcontrolnum, numdtl
FROM MHTEAM.DWDQ.INF_SENDPRO_CLAIMS_DQ_QA_7_QA
...
truncate table INF_B_SQL_ERROR;

select * from INF_B_SQL_ERROR;


select distinct filename
from MHTEAM.DWDQ.INF_B_SENDPRO_CLAIMS_DQ_7_837I_UNPIV_DETAIL
order by filename;
...
where filename = 
--'110088791b_pacdrd_12032025161702_sit_pd_30302539-9d7a-4a48-8f09-8ef9eaa7f71c.xml'
--'110031447a_pacdrd_03032025155959_test_de_5256a86b-e060-42c7-93eb-85e98a8b3b70.xml'
'110031447b_pacdrp_04082025130110_test_pd_587e39fa-99ef-4bdf-bd24-8fc9ab2573a1.xml'
