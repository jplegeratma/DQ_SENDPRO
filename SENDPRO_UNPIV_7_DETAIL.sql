...
SELECT RUN_DATE, FILENAME, CLAIM_TYPE, MEASURE, PAHdrSendingEntityID, TYPE, COUNT(TYPE) AS REC_CNT
FROM (
...
--  DI
    CASE
        ...
        SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ServProvNPI
...
--  Ex
    CASE  
		 ...
		 WHEN ( NOT EXISTS (SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ServProvNPI) )
         THEN 'INVALID'
...
--  DI
    CASE
        ...
        SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ServProvSecID
...
--  Ex
    CASE  
		 ...
		 WHEN ( NOT EXISTS (SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ServProvSecID) )
         THEN 'INVALID'
...
--  DI
    CASE
        ...
        SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = PrescriberNPI
...
--  Ex
    CASE  
		 ...
		 WHEN ( NOT EXISTS (SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = PrescriberNPI) )
         THEN 'INVALID'
...
--  DI
    CASE
        ...
        SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = PrescriberSecID
...
--  Ex
    CASE  
		 ...
		 WHEN ( NOT EXISTS (SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = PrescriberSecID) )
         THEN 'INVALID'
...
select *
FROM MHDWQA.SENDPRO.RAW_SPRO_NCPDP_COMPOUND_DTL cd
;


SELECT "AdjudicationDate" as AdjudicationDate, NVL(AdjudicationDate,TO_DATE('19700101','YYYYMMDD')) FROM MHDWQA.SENDPRO.RAW_SPRO_NCPDP_CLAIM i --WHERE 'DEMH8MKLM0300' = i."TransID"
;

SELECT TO_DATE('19700101','YYYYMMDD');

SELECT "AdjudicationDate" as AdjudicationDate, NVL(AdjudicationDate,'19700101') FROM MHDWQA.SENDPRO.RAW_SPRO_NCPDP_CLAIM i WHERE 'DEMH8MKLM001' = i."TransID"
;

SELECT NVL((SELECT "AdjudicationDate" as AdjudicationDate FROM MHDWQA.SENDPRO.RAW_SPRO_NCPDP_CLAIM i WHERE 'DEMH8MKLM001' = i."TransID"),'19700101')
;
