...
69	MPT_SENDPRO_ValidRevenueCode	1.	spro_b_enc_claim_inst_leg_hist AS i
                                    INNER JOIN mhdwqa.sendpro.spro_b_enc_inst_info_dtl_hist AS id ON
                                    i.clm_seq = id.clm_seq

	2.	IF id.REV_SEQ <> -1THEN 1 ELSE 0
...
CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O','M','H') THEN 'NOT APP'
...
68	MPT_SENDPRO_ValidEncInternalProvider	
1.	Join ENC_PRV_SEQ from Fact with sendpro. spro_b_enc_provider_hist on enc_prv_seq
2.	NPI: If ID_NPI  IS NOT  NULL Then 1 else 0
3.	Internal Provider Address Location: IF CDE_ENC_PROV_ID_LOC IS NOT NULL THEN 1 ELSE 0
4.	Internal Provider ID: IF ENC_PROV_ID IS NOT NULL THEN 1 ELSE 0
...
*/
    ...
    CASE 
         ...
         (
             (NOT EXISTS (SELECT prv.ID_PROVIDER_LOCATION from mhdwqa.SENDPRO.spro_b_enc_provider_hist as prv
         where SERVICING_ENC_PRV_SEQ = prv.ENC_PRV_SEQ AND prv.ID_PROVIDER_LOCATION IS NOT NULL AND prv.CDE_ENC_PROV_ID_LOC NOT IN ('#','+','-')))

         AND (NOT EXISTS (SELECT prv.ID_PROVIDER_LOCATION from mhdwqa.SENDPRO.spro_b_enc_provider_hist as prv
         where DTL_SERVICING_ENC_PRV_SEQ = prv.ENC_PRV_SEQ AND prv.ID_PROVIDER_LOCATION IS NOT NULL AND prv.CDE_ENC_PROV_ID_LOC NOT IN ('#','+','-')))
         )
         THEN 'INVALID'
         ELSE 'VALID' 
...
-- Changed to only 'I' on 1/28/2026
*/
...
DTL.DISCHARGE_DT removed 11/24/25
Changed to I and L and check for CDE_PATIENT_STATUS 1/9/26
*/
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I') THEN 'NOT APP'
        WHEN ( DISCHARGE_DT IS NULL OR DISCHARGE_DT = '1900-01-01') AND CDE_PATIENT_STATUS <> '30' THEN 'INVALID'
        ELSE 'VALID'
...
/* 
NEW 10/31/25
12.1.45	Pricing Method (MPT_SENDPRO_PricingMethod_837)
...
/*
12.1.49	Service Line Revenue Code (MPT_SENDPRO_SvcLineRevCode_837I)
•	837I Claims population: MPT_SENDPRO_ClaimType_837I
•	MPT_SENDPRO_ValidRevenueCode:  SPRO_B_ENC_INST_INFO_DTL_HIST. REV_SEQ

69	MPT_SENDPRO_ValidRevenueCode	1.	spro_b_enc_claim_inst_leg_hist AS i
                                    INNER JOIN mhdwqa.sendpro.spro_b_enc_inst_info_dtl_hist AS id ON
                                    id.clm_seq = id.clm_seq

	2.	IF id.REV_SEQ <> -1 THEN 1 ELSE 0
*/

    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O','H') THEN 'NOT APP'
         WHEN (DTL_REV_SEQ = -1) THEN 'INVALID'
         ELSE 'VALID'
...
select DISTINCT
    ...
    inst.CDE_ENC_ACO,
    inst.ID_SUBMITTER,
    DATE(inst.DOS_FROM_DT) AS DOS_FROM_DT,
    inst.CDE_CLM_TYPE,
    inst.CDE_CLM_STATUS,
    inst.CDE_CLM_DISPOSITION,
    inst.IND_OFFSET,
    DATE(inst.WH_FROM_DT) AS WH_FROM_DT,
    inst.MD_BATCH_SEQ,
