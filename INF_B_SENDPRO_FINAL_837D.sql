...
-- DROP TABLE MHTEAM.DWDQ.INF_B_SENDPRO_TARGET_837D;

-- CREATE TABLE MHTEAM.DWDQ.INF_B_SENDPRO_TARGET_837D AS

--TRUNCATE TABLE MHTEAM.DWDQ.INF_B_SENDPRO_TARGET_837D;

INSERT INTO MHTEAM.DWDQ.INF_B_SENDPRO_TARGET_837D
SELECT DISTINCT
    RUN_DATE,
    NUM_ICN,
    NUM_DTL,
...
CASE WHEN CDE_BILL_FREQ IS NULL OR CDE_BILL_FREQ IN ('#','+','-') THEN 'NULL'
...
CASE 
    WHEN CDE_CONTRACT_TYPE IS NULL OR CDE_CONTRACT_TYPE IN ('#','+','-') THEN 'NULL'
...
CASE
    ...
    WHEN AMT_PAID < 0 AND DTL_AMT_PAID < 0 THEN 'INVALID'
    ELSE 'VALID'
...
*/
    ...
    CASE 
         ...
         where DTL_BILLING_ENC_PRV_SEQ = prv.ENC_PRV_SEQ AND prv.CDE_ENC_PROV_ID_LOC IS NOT NULL AND prv.CDE_ENC_PROV_ID_LOC NOT IN ('#','+','-')))
         )
         THEN 'INVALID'
         ELSE 'VALID' 
...
/*
12.1.11	Servicing Provider Id (MPT_SENDPRO_ServicingProviderID_ALL)
•	ALL Claims population: MPT_SENDPRO_ClaimType_ALL
•	MPT_SENDPRO_ValidEncProvider: SPRO_B_ENC_CLAIM_DNTL_LEG_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_DNTL_INFO_DTL_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_CLAIM_PHRM_LEG_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_CLAIM_INST_LEG_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_INST_INFO_DTL_HIST. BILLING_ENC_PRV_SEQ: 
*/

    CASE  
         WHEN (servicing_ProviderInternalId IS NULL OR servicing_ProviderInternalId IN ('#','+','-')) AND (dtl_servicing_ProviderInternalId IS NULL OR dtl_servicing_ProviderInternalId IN ('#','+','-')) THEN 'NULL'
		 WHEN 
         (
...
/*
12.1.12	Servicing Provider NPI (MPT_SENDPRO_ServicingProviderNPI_ALL)
•	ALL Claims population: MPT_SENDPRO_ClaimType_ALL
...
/*
12.1.14	Servicing Provider Location (MPT_SENDPRO_ServicingProviderLoc_ALL)
•	ALL Claims population: MPT_SENDPRO_ClaimType_ALL
•	MPT_SENDPRO_ValidEncProvider: SPRO_B_ENC_CLAIM_DNTL_LEG_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_DNTL_INFO_DTL_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_CLAIM_PHRM_LEG_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_CLAIM_INST_LEG_HIST. BILLING_ENC_PRV_SEQ, SPRO_B_ENC_INST_INFO_DTL_HIST. BILLING_ENC_PRV_SEQ: 
*/
    CASE 
...
/*
12.1.15	Servicing Provider Taxonomy Code (MPT_SENDPRO_ServicingProviderTaxonomyCode_ALL)
•	ALL Claims population: MPT_SENDPRO_ClaimType_ALL
...
/*
12.1.19	Admission Date (MPT_SENDPRO_Admission_Date_837I)
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O','M') THEN 'NOT APP'
        WHEN ADMIT_DT IS NULL THEN 'NULL'
        WHEN ADMIT_DT = '1900-01-01' THEN 'INVALID'
        ELSE 'VALID'
...
*/
    ...
    CASE 
        ...
        WHEN QTY_UNITS_BILLED <= 0 AND DTL_QTY_UNITS_BILLED <= 0 THEN 'INVALID'
        ELSE 'VALID'
    END AS QuantityBilled1X,

/*
12.1.22	Admitting Diagnosis (MPT_SENDPRO_Admitting_Diagnosis_837I)
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O') THEN 'NOT APP'
         WHEN DIAGRP_SEQ IS NULL OR DIAGRP_SEQ IN (-1,-4,-5) THEN 'NULL'
...
/*
12.1.23	Primary Diagnosis (MPT_SENDPRO_Primary_Diagnosis_837)
...
/*
12.1.24	Discharge Date (MPT_SENDPRO_Discharge_Date_837I)
•	837I Claims population: MPT_SENDPRO_ClaiimType_837I_INP
...
•	Valid Date value parameter: MPT_SENDPRO_DateIsNotNull_ALL: SPRO_B_ENC_CLAIM_INST_LEG_HIST. DISCHARGE_DT_TM, SPRO_B_ENC_INST_INFO_DTL_HIST. DISCHARGE_DT
*/
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O','H') THEN 'NOT APP'
        WHEN DISCHARGE_DT IS NULL AND DTL_DISCHARGE_DT IS NULL THEN 'NULL'
        WHEN DISCHARGE_DT = '1900-01-01' AND DTL_DISCHARGE_DT = '1900-01-01' THEN 'INVALID'
        ELSE 'VALID'
    END AS DischargeDate1X,
/*
12.1.25	Type of Admission (MPT_TypeofAdmission_837I)
...
/*
12.1.26	Source of Admission (MPT_SourceofAdmission_837I)
•	837I Claims population: MPT_SENDPRO_ClaimType_837I_LTC, MPT_SENDPRO_ClaiimType_837I_INP
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O') THEN 'NOT APP'
        WHEN CDE_ADMIT_SOURCE IS NULL OR CDE_ADMIT_SOURCE IN ('#','+','-') THEN 'NULL'
...
/*
12.1.27	Patient Status Code (MPT_SENDPRO_PatientStatusCode_837I)
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O') THEN 'NOT APP'
        WHEN CDE_PATIENT_STATUS IS NULL OR CDE_PATIENT_STATUS IN ('#','+','-') THEN 'NULL'
...
/*
12.1.28	Facility Type Code (MPT_SENDPRO_FacilityTypeCode_837I_837P)
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O') THEN 'NOT APP'
        WHEN CDE_TYPE_OF_BILL IS NULL OR CDE_TYPE_OF_BILL IN ('#','+','-') THEN 'NULL'
...
/*
12.1.30	Procedure Modifier Code (MPT_SENDPRO_ProcedureModCode_837)
•	837I Claims population: MPT_SENDPRO_ClaiimType_837I_INP, MPT_SENDPRO_ClaiimType_837I_OUTP, MPT_SENDPRO_ClaiimType_837I_LTC, MPT_SENDPRO_ClaiimType_837P, MPT_SENDPRO_ClaiimType_837D
...
*/
    ...
    CASE 
        WHEN PROCMFRGRP_SEQ IS NULL OR PROCMFRGRP_SEQ IN (-1,-4,-5) THEN 'NULL'
...
/*
12.1.31	Place of Service (MPT_SENDPRO_PlaceOfServiceCode_837)
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('M','D') THEN 'NOT APP'
         WHEN CDE_PLACE_OF_SERVICE IS NULL OR CDE_PLACE_OF_SERVICE IN ('#','+','-') THEN 'NULL'
...
/* 
NEW 10/31/25
12.1.46	Tooth Number (MPT_SENDPRO_ToothNumber_837D)
•	837I Claims population: MPT_SENDPRO_ClaimType_837P, MPT_SENDPRO_ClaiimType_837D
•	Missing String Value Parameter: MPT_StringIsNull_ALL, SPRO_B_ENC_DNTL_ATTRIBUTE_M2M. cde_tooth_nbr

This is a new field that need to be validated. Query below for reference
select a.num_icn,b.clm_dtl_seq, c.cde_tooth_nbr
from SPRO_B_ENC_CLAIM_DNTL_LEG_HIST a join SPRO_B_ENC_DNTL_INFO_DTL_HIST b on
a.clm_seq = b.clm_seq
left join SPRO_B_ENC_DNTL_ATTRIBUTE_M2M c on
b.clm_dtl_seq = c.clm_dtl_seq;
...
*/
    ...
    CASE WHEN CDE_CLM_TYPE NOT IN ('D') THEN 'NOT APP'
         ...
         THEN 'INVALID'
         ELSE 'VALID'
    END AS ToothNumber1X,

/* 
12/19/25

12.1.47	Service Category (MPT_SENDPRO_ServiceCategory_837)
•	837I Claims population: MPT_SENDPRO_ClaimType_837P, MPT_SENDPRO_ClaiimType_837D, MPT_SENDPRO_ClaiimType_837I
•	Missing String Value Parameter: MPT_StringIsNull_ALL, SPRO_B_ENC_INST_INFO_DTL_HIST. CDE_ENC_SVC_CAT, SPRO_B_ENC_PROF_INFO_DTL_HIST. CDE_ENC_SVC_CAT,
SPRO_B_ENC_DNTL_INFO_DTL_HIST. CDE_ENC_SVC_CAT

*/

    CASE WHEN CDE_CLM_TYPE NOT IN ('L','I','O','H','M','D') THEN 'NOT APP'
         WHEN (DTL_CDE_ENC_SVC_CAT IS NULL OR DTL_CDE_ENC_SVC_CAT IN ('#','+','-'))
         THEN 'INVALID'
         ELSE 'VALID'
    END AS ServiceCategory1X,

/*
12/19/25

12.1.48	Oral Category Description (MPT_SENDPRO_OralCatgDesig_837D)
•	837I Claims population: MPT_SENDPRO_ClaimType_837D
•	Missing String Value Parameter: MPT_StringIsNull_ALL, SPRO_B_ENC_DNTL_INFO_DTL_HIST. CDE_ORAL_CAVITY_DESIG_01

*/

    CASE WHEN CDE_CLM_TYPE NOT IN ('D') THEN 'NOT APP'
         WHEN DTL_CDE_ORAL_CAVITY_DESIG_01 IS NULL OR DTL_CDE_ORAL_CAVITY_DESIG_01 IN ('#','+','-',' ') THEN 'NULL'
         ELSE 'VALID'
    END AS OralCategoryDesc1X,

/*
12/19/25

12.1.50	Tooth Surface Code (MPT_SENDPRO_ToothSurfaceCode_837D)
•	837I Claims population: MPT_SENDPRO_ClaimType_837D
•	Missing String Value Parameter: MPT_StringIsNull_ALL, SPRO_B_ENC_DNTL_ATTRIBUTE_M2M. CDE_TOOTH_SURFACE_01
 */

    CASE WHEN CDE_CLM_TYPE NOT IN ('D') THEN 'NOT APP'
         WHEN DTL_CLM_DTL_SEQ IS NULL OR DTL_CLM_DTL_SEQ IN (-1,-4,-5) THEN 'NULL'
         WHEN NOT EXISTS (SELECT CDE_TOOTH_SURFACE_01 from MHDWQA.SENDPRO.SPRO_B_ENC_DNTL_ATTRIBUTE_M2M WHERE DTL_CLM_DTL_SEQ = CLM_DTL_SEQ
            AND CDE_TOOTH_SURFACE_01 IS NOT NULL AND CDE_TOOTH_SURFACE_01 NOT IN ('#','+','-',' '))
         THEN 'INVALID'
         ELSE 'VALID'
    END AS ToothSurfaceCode1X,
...
select DISTINCT
    ...
    dntl.CDE_ENC_ACO,
    dntl.ID_SUBMITTER,
    DATE(dntl.DOS_FROM_DT) AS DOS_FROM_DT,
    dntl.CDE_CLM_TYPE,
    dntl.CDE_CLM_STATUS,
    dntl.CDE_CLM_DISPOSITION,
    dntl.IND_OFFSET,
    DATE(dntl.WH_FROM_DT) AS WH_FROM_DT,
    dntl.MD_BATCH_SEQ,
    ...
    dntl.AMT_PAID,
    dntl.AMT_BILLED,
    DATE(dntl.DOS_TO_DT) AS DOS_TO_DT,
    NULL AS ADMIT_DT,
...
--    dtl.CDE_CONTRACT_TYPE       AS DTL_CDE_CONTRACT_TYPE,
    ...
    DATE(dtl.DOS_FROM_DT)       AS DTL_DOS_FROM_DT,
    DATE(dtl.DOS_TO_DT)         AS DTL_DOS_TO_DT,
    ...
    dtl.CDE_ENC_SVC_CAT         AS DTL_CDE_ENC_SVC_CAT,
...
AND   dtl.IND_OFFSET = 'N'
AND   dntl.MD_BATCH_SEQ NOT IN ( SELECT DISTINCT tar.MD_BATCH_SEQ from MHTEAM.DWDQ.INF_B_SENDPRO_TARGET_837D tar)
 ) A;
