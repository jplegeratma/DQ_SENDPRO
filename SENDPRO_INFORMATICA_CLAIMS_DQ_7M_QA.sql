...
--  DI
    CASE
        ...
        SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ord_OrderingProvID
...
--  Ex
    ...
    --all Claim_Types
		 ...
		 WHEN ( NOT EXISTS (SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ord_OrderingProvID) )
         THEN 'INVALID'
...
--  DI
    ...
    --all Claim_Types
        ...
        SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ord_ProviderInternalId
...
--  Ex
    ...
    --all Claim_Types
		 ...
		 WHEN ( NOT EXISTS (SELECT ENC_PROV_ID from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist where ENC_PROV_ID NOT IN ('#','+','-') AND ENC_PROV_ID = ord_ProviderInternalId) )
         THEN 'INVALID'
...
-- DI
    CASE
        ...
        AND ord_ProviderPidsl IN (SELECT enc_prov_id from MHDWDEV.SENDPRO.spro_b_enc837_provider_hist ap 
--          WHERE enc_prov_id NOT IN ('#','+','-') 
